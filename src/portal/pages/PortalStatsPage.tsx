...
import { type TeamMemberRole } from "../lib/mockPortal";
import { PortalPageHeader } from "../components/PortalPageHeader";
import { usePortalSession } from "../lib/session";

const roleOptions: TeamMemberRole[] = ["Owner", "Manager", "Analyst", "Billing Admin"];

export function PortalStatsPage() {
  const { inviteTeamMember, team, updateTeamMemberRole, toggleTeamMemberStatus } = usePortalSession();
            ...
            <form className="mt-6 grid gap-4" onSubmit={handleInvite}>
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-4 text-white outline-none transition placeholder:text-white/30 focus:border-emerald-300/50"
                    placeholder="casey@harborfit.co"
                  />
                </label>
              </div>
              ...
              <label className="space-y-2">
                ...
                <select
                  ...
                  className="w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-4 text-white outline-none transition focus:border-emerald-300/50"
                >
              ...
              <button
                type="submit"
                className="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-emerald-400 to-sky-500 px-6 py-4 text-sm font-semibold text-slate-950 shadow-glow transition duration-300 hover:scale-[1.01] hover:opacity-95"
              >
                Send invite
              </button>
            </form>
            ...
            {team.map((member) => (
              <div key={member.id} className="rounded-[1.5rem] border border-white/10 bg-black/20 p-5">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
                  <select
                    value={member.role}
                    onChange={(event) => void updateTeamMemberRole(member.id, event.target.value as TeamMemberRole)}
                    className="w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-4 text-white outline-none transition focus:border-emerald-300/50"
                  ...
                  <button
                    type="button"
                    onClick={() => void toggleTeamMemberStatus(member.id)}
                    disabled={member.role === "Owner"}
                    className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/15 bg-white/5 px-5 py-4 text-sm font-semibold text-white transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-40"
                  >
